"""
Erstellt ein KDP-fertiges Word-Dokument aus den Markdown-Kapiteldateien fuer Band 2.
"""
import re
import os
...
CHAPTER_FILES = [
    f"Die_Herrenhaus_Detektive_Band2_Kapitel{i}.md" for i in range(1, 20)
]

# Kapiteltitel
CHAPTER_TITLES = {
    1: "Der versiegelte Brief",
    2: "Die Wahrheit \u00fcber den Brunnen",
    3: "Die vermissten Kinder",
    4: "Geheimtinte",
    5: "Die Zeugin",
    6: "Zur\u00fcck in den Tunnel",
    7: "Meiers Gest\u00e4ndnis",
    8: "Das R\u00e4tsel der vier Linien",
    9: "Die Treppe",
    10: "Die falsche Abzweigung",
    11: "Eingesperrt",
    12: "Die Quelle",
    13: "Bens Plan",
    14: "Der Aufstieg",
    15: "Die Zeitkapsel",
    16: "Frau Bergmanns Tr\u00e4nen",
    17: "Das richtige Erbe",
    18: "Der Anruf",
    19: "Bald",
}
...
def add_impressum_page(doc):
    ...
    lines = [
        "Die Herrenhaus-Detektive, Band 2: Das Geheimnis des Brunnens",
        "",
        "\u00a9 2026 Benjamin Krug",
...
def add_dedication_page(doc):
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(
        "F\u00fcr alle, die nicht aufh\u00f6ren zu suchen \u2014\n"
...
def parse_markdown_chapter(filepath):
    ...
    # Entferne die Buch-Header (# Die Herrenhaus-Detektive, ## Band 2, ---)
    lines = content.split('\n')
    ...
    for line in lines:
        ...
        if chapter_started:
            # Stoppe vor "Ende von Band 2" Markierungen am Dateiende
            if line.strip().startswith('*Ende von Band') or line.strip().startswith('*Jonas, Mila und Ben'):
                continue
...
def add_end_page(doc):
    """Erstellt die emotionale Schlussseite mit Dank und Ueberleitung zu Band 3."""
    add_page_break(doc)
    ...
    lines = [
        ...
        "",
        "Jonas, Mila und Ben haben die Quelle unter dem Dorf gefunden.",
        "Sie haben Frau Bergmanns Geheimnis gel\u00fcftet.",
        "Und ein Anruf hat alles ver\u00e4ndert.",
        "",
        "Aber die Geschichte ist noch nicht vorbei \u2026",
        "",
        "Im Wald wartet ein neues Geheimnis.",
        "Eine zweite Quelle, die niemand finden konnte.",
        "Und jemand kommt endlich nach Hause.",
        "",
        "Bist du bereit f\u00fcr das gr\u00f6\u00dfte Abenteuer?",
    ]
    ...
    for line in lines:
        ...
        if line:
            ...
            if line == "Hat dir das Buch gefallen?":
                run.font.bold = True
                run.font.size = Pt(14)
            if line == "Bist du bereit f\u00fcr das gr\u00f6\u00dfte Abenteuer?":
                run.font.bold = True
                run.font.italic = True
        p.paragraph_format.space_after = Pt(2)
...
BAND3_KAPITEL1 = os.path.join(
    BASE_DIR, "..", "..", "Band_3", "Linear", "Kapitel",
    "Die_Herrenhaus_Detektive_Band3_Kapitel1.md")

# Bis hierhin (einschliesslich) laeuft die Leseprobe -- endet auf dem Cliffhanger.
BAND3_LESEPROBE_ENDE = "Beide zeigten zum Wald."


def _lade_band3_leseprobe():
    """Liest den ECHTEN Anfang von Band 3, Kapitel 1 -- Titel + Text bis zum
    Cliffhanger. Kein abgetippter Text: die Vorschau bleibt so automatisch
    synchron mit dem geschriebenen Band 3."""
    with open(BAND3_KAPITEL1, "r", encoding="utf-8") as f:
        lines = f.read().split("\n")
    ...
    for line in lines:
        ...
        if gestartet and not fertig:
            prosa.append(line)
            if line.strip() == BAND3_LESEPROBE_ENDE:
                fertig = True
                break

    # In Absatzgruppen aufteilen (Leerzeile trennt), "---" = Szenentrenner
    gruppen, aktuell = [], []
    for line in prosa:
        s = line.strip().replace(" -- ", " — ")   # sauberer Gedankenstrich
        if not s:
            if aktuell:
...
def add_band3_teaser(doc):
    """Vorschau auf Band 3: eine echte Leseprobe aus dem geschriebenen Kapitel 1."""
    add_page_break(doc)
    doc.add_paragraph()
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Die Herrenhaus-Detektive, Band 3")
    run.font.size = Pt(13)
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(2)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Die zweite Quelle")
    run.font.size = Pt(18)
    run.font.bold = True
    run.font.name = 'Georgia'
    p.paragraph_format.space_after = Pt(18)
    ...
    # Kapitel-Ueberschrift der Leseprobe
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    # Echter Text -- als Buch-Fliesstext (links, mit Kursiv-Erkennung)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Das dritte gro\u00dfe Abenteuer der Herrenhaus-Detektive.")
    run.font.size = Pt(11)
    run.font.name = 'Georgia'
...
def add_series_overview_page(doc):
    ...
    _add_series_block(
        ...
        band_info=[
            ...
            "Band 3: Die zweite Quelle \u2014 Erscheint bald!",
        ],
    )
...
def main():
    print("Erstelle KDP-Manuskript fuer Band 2...")
    doc = setup_document()

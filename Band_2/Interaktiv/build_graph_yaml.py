...
def cluster_of(sid: str) -> str:
    ...
    if 41 <= num <= 59:
        return "B"          # Das Wasser
    if 66 <= num <= 87:
        return "C"          # Die Zeitzeugin
    if 91 <= num <= 106:
        return "D"          # Winters Geheimnis
    if num >= 200:
        return "SPECIAL"    # Geheim-Ende / Easter Egg
    return "?"
...
def parse_section(path: Path):
    ...
    if m_ende:
        ...
        # Geheim-Ende (Codewort-System) nicht ueber Graph-Kante erreichbar
        if int(re.match(r"\d+", sid).group()) >= 200:
            node["codewort_system"] = True
...
def build():
    ...
    graph = {
        "metadata": {
            "title": "Das Geheimnis des Brunnens — Entscheidungsbuch",
